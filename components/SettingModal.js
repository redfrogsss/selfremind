...
export default function SettingModal(props) {
    ...
    const submitHandler = (e) => {
        ...
        if (e.target.newusername.toString().length === 0) {
            if (e.target.repeatnewpassword.value !== undefined | e.target.newpassword.value !== undefined) {
                if (e.target.repeatnewpassword.value !== e.target.newpassword.value) {
                    FailToast("New password is not matched.");
                    return;
                }
                if (e.target.oldpassword.value.toString().length === 0) {
        ...
        var data = {
            username: e.target.newusername.value.toString().length > 0 ? e.target.newusername.value.toString() : undefined,
            pwd: e.target.repeatnewpassword.value.toString().length > 0 ? e.target.repeatnewpassword.value.toString() : undefined,
        }
        var pwd_old = e.target.oldpassword.value.toString().length > 0 ? e.target.oldpassword.value.toString() : undefined;

        axios.get("/api/user/" + cookies.userID).then((res) => {
            const username = res.data.result[0].username;
            axios.post("/api/login", { username: username, pwd: pwd_old })
                .then((res) => {
                    if (res.data.authStatus === true) {
                        axios.put("/api/user/" + cookies.userID, data)
                            .then((res) => {
                                SuccessToast();
                                onClose();
                            })
